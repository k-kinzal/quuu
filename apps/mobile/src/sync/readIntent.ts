import { isAddAction } from './addAction.js'
import { asRecord, bool, fail, intentVersionOk, num, str, type ParseResult } from './json.js'
...
export function parseIntent(text: string): ParseResult<SyncIntent> {
  ...
  if (typeof o.id !== 'string' || !o.id) return fail('id is missing')
  const version = num(o.version, -1)
  const op = parseOp(o.op, version)
  if (!op) return fail('cannot read op')

  if (typeof o.device !== 'string' || !o.device || typeof o.seq !== 'number' || !Number.isInteger(o.seq) || o.seq < 0 || typeof o.createdAt !== 'string' || typeof o.baseRev !== 'number' || !Number.isInteger(o.baseRev) || o.baseRev < 0) return fail('a required intent field is missing')
  if (o.expect != null && (typeof o.expect !== 'object' || !isTaskStatus((o.expect as Record<string, unknown>).status) || typeof (o.expect as Record<string, unknown>).updatedAt !== 'string' || typeof (o.expect as Record<string, unknown>).runSeq !== 'number' || !Number.isInteger((o.expect as Record<string, unknown>).runSeq) || Number((o.expect as Record<string, unknown>).runSeq) < 0)) return fail('cannot read the approval target')
