...
| Change report | When a task reaches review, a **separate agent writes what changed a level above the code** — which concept became which, which relationship moved, which rule now says something else — as a static HTML page kept in Quuu's data area. Every claim carries a path, a commit or a PR. Read from the task's Report surface. A task that reaches review again gets a new report only when the worktree changed since the last one; "write again" on the surface always writes |
